...
var RhinoCompute = {
    ...
    Intersection : {
        ...
        meshMesh : function(meshes, tolerance, mode, performPreprocessing, textLog, multiple=false) {
            let url="rhino/geometry/intersect/intersection/meshmesh-mesharray_double_setscombinations_bool_fileio.textlog";
            if(multiple) url = url + "?multiple=true"
            let args = RhinoCompute.zipArgs(multiple, meshes, tolerance, mode, performPreprocessing, textLog);
            var promise = RhinoCompute.computeFetch(url, args);
            return promise;
        },
    ...
    Grasshopper: {
        ...
        evaluateDefinition : function(definition, trees) {
            ...
            //
            // Args:
            //     definition (str/buffer): contents of .gh/.ghx file
            ...
            // console.log(definition.constructor)

            // base64 encode binary/xml grasshopper definition
            if (definition.constructor === Uint8Array) {
                definition = base64Uint8Array(definition)
            } else {
                // strip utf-8/16 bom
                if (definition.charCodeAt(0) === 0xFEFF) {
                    definition = definition.slice(1);
                }
                definition = btoa(definition);
            }
...
if (_is_node && typeof btoa !== 'function')
{
    // function btoa(str) {
    //     let buffer;
    //     // console.log(str instanceof Buffer);
    ...
    //         // }
    //     } else {
    //         // strip utf-8 bom - buffer-to-string conversion
    //         // translates EFBBBF (UTF-8 BOM) to FEFF (UTF-16 BOM)
    //         // TODO: handle on server
